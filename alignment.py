...
def alignPair(f1, f2, matches, m, nRANSAC, RANSACthresh):
    ...
    for i in range(nRANSAC):
        ...
        tempInlierIndices = getInliers(f1, f2, matches, matrix, RANSACthresh)
        if len(tempInlierIndices) > len(inliers):
            inliers = tempInlierIndices
            M = matrix
...
def leastSquaresFit(f1, f2, matches, m, inlier_indices):
    ...
    if m == eTranslate:
        ...
        validMatches = []
        for i in range(len(inlier_indices)):
            validMatches.append(matches[inlier_indices[i]])
        M = computeHomography(f1, f2, validMatches)
        #TODO-BLOCK-END
        #END TODO
